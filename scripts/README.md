...
| 脚本 | 用途（一句话） | 谁调用它 | 用法 |
|---|---|---|---|
| `build_ios_no_codesign.sh` | 无签名构建 iOS 应用（debug/release），iOS 出包的统一入口 | 本机开发；CI `ios-release-ipa.yml` | `bash scripts/build_ios_no_codesign.sh [debug\|release]` |
...
本机 / CI 出 iOS 包
  build_ios_no_codesign.sh
    ...
    ├─ (SDK) build-player-ffi-xcframework.sh
