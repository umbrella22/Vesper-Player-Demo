...
| mode 9 BAS | 不支持 | 不支持 | 获取后分类并丢弃；定论永久不支持（§1.1） |
...
### 1.1 mode 9 定论与依据（2026-09-03 复核）

mode 9 BAS 定论为**永久不实现**，不保留 backlog。依据：

- **开源侧冻结**：官方仓库 `bilibili/bas`（含文档站 `bilibili.github.io/bas`）最后一次
  提交为 2020-03-23；2017-12 至 2020-03 全程仅 10 次提交、单一作者（DIYgod，时任播放器
  组），无 release，19 个 open issue 无人处理。社区脚本引擎 `hozuki/sebas` 亦已停更。
- **渲染端只有 web**：`bas.js`（gh-pages 分支）是纯 JS 运行时，导出名 `BasDanmaku`，
  语法为 `DefText` / `DefButton` / `DefPath` 等声明式定义函数——无任何移动端形态的
  官方实现。
- **官方 Android 客户端实测无渲染器**：对 `tv.danmaku.bili` 9.8.0
  （versionCode 9080300）全部 31 个 dex 做字符串级扫描（方法有效性经
  `api.bilibili.com` 203 处、`danmaku` 19605 处、`DmSegMobile` 43 处命中验证），
  BAS 特征全部为零：`com/bilibili/bas/` 类路径、`BasDanmaku`、`bas.js`、
  `DefText` / `DefButton` / `DefPath`、`enableBas`；assets 内亦无 bas 相关文件
  （`CreateText` 命中属表情包/纹理/Toast，`SetExpr` 命中属 Lynx JNI，均与 BAS 无关）。
- **数据管道保留但未消费**：该客户端弹幕走 gRPC 风格的
  `bilibili.community.service.dm.v1` 协议，`DmWebViewReply` 仍保留 `specialDms`
  字段——官方 App 取回 mode 9 数据后在手机端同样丢弃、不渲染。

因此本应用的处置（获取后分类并丢弃）与官方移动端行为一致，不构成体验差距；mode 7
声明式子集反而是移动端超出官方手机客户端的能力（web 播放器待遇）。除非未来官方
移动端出现 BAS 渲染迹象（dex 中出现上述特征串），该定论无需重开。
...
flowchart TD
  ...
  U --> H["规范化为 HTTPS 并校验 Bilibili 资源域名"]
...
- HTTPS 绝对 URL；
- `//host/path` 网络路径引用，或白名单 Bilibili host 的旧 HTTP URL；这两种形态在请求前升级为 HTTPS；
- host 必须为 `bilibili.com`、`hdslb.com` 或其子域；
- URL 不得包含 user-info。

公开响应中存在 `i0.hdslb.com/bfs/dm/...bin`、`comment.bilibili.com` 和
`api.bilibili.com` 形态，其中部分旧响应仍使用 `http://i0.hdslb.com/...`。特殊资源的实际
传输始终使用 HTTPS，并携带当前视频 Referer，但不携带 Cookie；拒绝或加载失败时只记录
校验原因、host 或异常类型，不记录可能带签名参数的完整 URL。

2026-09-03 使用 `BV16a4y1v7L2` 做过真实链路复核：View 返回
`http://i0.hdslb.com/bfs/dm/...bin`，规范化后的 HTTPS 请求成功；资源包包含 20 条 mode 9
BAS、没有 mode 7。该样本只证明特殊资源传输与分类通过，不代表 BAS 视觉渲染通过；按照
当前安全边界，这 20 条 payload 仍会被丢弃。
...
文案和手机端交互；TV 不接入这些能力。契约、证据等级与实施前验证规程集中在
`doc/bili-interaction-api-notes.md`，本节只保留链路影响。
...
点赞状态需要独立于渲染事件维护，避免一次互动导致整个分段重新布局。候选路径与参数名
存在多形态，需按 `doc/bili-interaction-api-notes.md` §10 实测确认后再定稿。
