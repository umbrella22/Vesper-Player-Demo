...
void _registerPlaybackCommentsWidgetTests() {
  for (final backAction in ['system', 'player', 'panel']) {
    testWidgets(
      'comment replies $backAction back returns to comments before leaving playback',
      (tester) async {
        await _pumpPlaybackPage(
          tester,
          surfaceSize: const Size(390, 844),
          previousPage: const Scaffold(body: Text('播放前页面')),
        );
        await tester.pumpAndSettle();
        await tester.tap(find.text('评论 78'));
        await tester.pumpAndSettle();
        await tester.ensureVisible(find.text('共3条回复 >'));
        await tester.pumpAndSettle();
        final comments = find.byKey(
          const PageStorageKey<String>('playback-comments'),
        );
        ...
        expect(find.text('播放前页面'), findsOneWidget);
      },
      variant: TargetPlatformVariant.only(TargetPlatform.macOS),
    );
  }

  testWidgets('leaving the comments tab removes the replies back handler', (
    tester,
  ) async {
    await _pumpPlaybackPage(
      tester,
  ...
  testWidgets('playback comment replies load the next page near the list end', (
    WidgetTester tester,
  ) async {
    ...
    expect(find.textContaining('楼中楼第21条', findRichText: true), findsOneWidget);
  }, variant: TargetPlatformVariant.only(TargetPlatform.macOS));

  testWidgets('mobile comment replies stay below the playing video', (
    WidgetTester tester,
  ) async {
    await _pumpPlaybackPage(
      tester,
    ...
    await tester.pumpAndSettle();
    final modalBarrierCountBefore = find.byType(ModalBarrier).evaluate().length;
    await tester.tap(find.text('共3条回复 >'));
    await tester.pumpAndSettle();

    final stageRect = tester.getRect(find.byType(vesper_ui.VesperPlayerStage));
  ...
  testWidgets('playback info tabs switch with horizontal swipes', (
    WidgetTester tester,
  ) async {
    await _pumpPlaybackPage(tester, surfaceSize: const Size(390, 640));
  ...
  testWidgets('playback comments load more near the bottom', (
    WidgetTester tester,
  ) async {
    final harness = await _pumpPlaybackPage(
      tester,
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      // 播放页现在有两个输入框（弹幕与评论），按提示文案定位评论输入框。
      final commentField = find.widgetWithText(TextField, '你猜我的评论区在等谁？');
      await tester.tap(commentField);
      await tester.enterText(commentField, '好看，支持一下');
  ...
  testWidgets('playback settings omit system playback controls', (
    WidgetTester tester,
  ) async {
  ...
  testWidgets('playback settings select an SDK external subtitle track', (
    WidgetTester tester,
  ) async {
  ...
  testWidgets('playback speed panel applies and restores playback rate', (
    WidgetTester tester,
  ) async {
    final harness = await _pumpPlaybackPage(tester);
  ...
  testWidgets('playback page lays out at compact phone size without overflow', (
    WidgetTester tester,
  ) async {
  ...
  testWidgets('dlna load failure disconnects and keeps picker open', (
    WidgetTester tester,
  ) async {
    tester.binding.defaultBinaryMessenger.setMockMethodCallHandler(
  ...
  testWidgets('tv playback locks landscape and touch toggles controls', (
    WidgetTester tester,
  ) async {
    final platformCalls = <MethodCall>[];
    tester.binding.defaultBinaryMessenger.setMockMethodCallHandler(
